...
Clinical \ac{EEG} data represents an invaluable but underutilized resource for machine learning research. While hundreds of thousands of \ac{EEG} recordings are collected annually in hospitals worldwide, relatively few are available to the research community in formats suitable for algorithm development \cite{obeid2016temple}. This scarcity is particularly problematic given that modern deep learning approaches require massive amounts of diverse training data to achieve robust performance.

The \ac{TUH} \ac{EEG} Corpus was developed to address this gap, providing the research community with access to large-scale clinical \ac{EEG} data. The complete \ac{TUH} corpus comprises 16,986 sessions from 10,874 unique subjects collected between 2002 and the present, representing approximately 29.1 years of cumulative \ac{EEG} data \cite{obeid2016temple}. Importantly, this is clinical-grade data collected in real-world hospital settings including the \ac{ICU}, epilepsy monitoring unit, emergency room, and outpatient services, rather than tightly controlled laboratory conditions.

The \ac{TUEV} subset was created specifically for event classification tasks and contains sessions known to include clinically significant events such as epileptiform discharges, artifacts, and eye movements \cite{harati2015improved}. This focused subset enables targeted development and evaluation of algorithms for automatic event detection and classification, a critical capability for assisting neurologists in analyzing long-term \ac{EEG} recordings.

\subsection{Dataset Structure and Organization}
\label{sec:dataset_structure}

The \ac{TUEV} dataset is organized into two primary partitions: a training set for model development and an evaluation set for independent testing. The training set contains 359 \ac{EEG} files, while the evaluation set comprises 159 files. This separation ensures that models can be evaluated on data from sessions not seen during training.

Signal data is stored in the \ac{EDF}, an open-source format that encodes multi-channel time-series data using 16-bit integers. Each \ac{EDF} file contains both the raw \ac{EEG} signals and associated metadata in an ASCII header describing channel labels, sampling rates, and recording parameters. The signal data represents pruned recordings where sections marked as uninformative by attending technicians were removed, resulting in focused segments containing relevant neural activity.
...
The distribution of events varies significantly across the training and evaluation sets, reflecting the natural distribution of events in clinical practice. In the training set, artifact events are most prevalent (164 files), followed by background (211 files), while clinical events like \ac{SPSW} are less common (27 files). The evaluation set shows similar patterns with artifacts in 46 files and \ac{SPSW} in only 9 files.
...
\begin{figure}[htbp]
    ...
    \caption{International 10-20 electrode system showing the standardized placement of electrodes on the scalp. The system uses anatomical landmarks (nasion, inion, and preauricular points) to ensure consistent electrode positioning across subjects.}
...
\begin{itemize}
    ...
    \item \textbf{Clinical Consistency}: Bipolar montages match the representation neurologists use for visual analysis, facilitating clinical validation
    \item \textbf{Spatial Localization}: Local voltage gradients provide better spatial resolution for identifying event origins
\end{itemize}
...
\begin{itemize}
    ...
    \item Deterministic algorithms are enabled where computational efficiency allows
...
\textbf{AUC-PR Macro} focuses on positive class performance through precision-recall curves, particularly important for rare clinical events:
\begin{equation}
\text{AUC-PR}_{macro} = \frac{1}{K} \sum_{i=1}^{K} \int_0^1 Precision_i(Recall_i) \, dRecall_i
\end{equation}

One-vs-rest precision-recall curves are computed for each class and then macro-averaged.
