...
Maximilian Beck* ${ }^{1,2,3}$<br>Korbinian Pöppel ${ }^{* 1,2,3}$<br>Markus Spanring ${ }^{1}$<br>Andreas Auer ${ }^{1,2}$ Oleksandra Prudnikova ${ }^{1}$<br>Michael Kopp<br>Günter Klambauer ${ }^{1,2,3}$ Johannes Brandstetter ${ }^{1,2,3}$ Sepp Hochreiter ${ }^{1,2,3}$<br>*Equal contribution<br>${ }^{1}$ ELLIS Unit, LIT AI Lab, Institute for Machine Learning, JKU Linz, Austria<br>${ }^{2}$ NXAI Lab, Linz, Austria, ${ }^{3}$ NXAI GmbH, Linz, Austria
...
**Images on this page:** 1
- Image ID: img-0.jpeg (Position: 277,1315 to 1421,1830)
...
$c_{t}=\mathrm{f}_{t} \quad c_{t-1}+\mathrm{i}_{t} \quad z_{t}, \quad h_{t}=\mathrm{o}_{t} \psi\left(c_{t}\right)$.

The constant error carousel is the additive update of the cell state $c_{t-1}$ (green) by cell inputs $z_{t}$ and moderated by sigmoid gates (blue). The input gate $\mathrm{i}_{t}$ and the forget gate $\mathrm{f}_{t}$ control this update, while the output gate $\mathrm{o}_{t}$ controls the output of the memory cell, i.e. the hidden state $h_{t}$. The cell state is normalized or squashed by $\psi$ and then output gating gives the hidden state.
...
**Images on this page:** 1
- Image ID: img-1.jpeg (Position: 739,1053 to 1370,1355)
...
& h_{t}=\mathrm{o}_{t} \tilde{h}_{t}, \quad \tilde{h}_{t}=\psi\left(\tilde{c}_{t}\right) \quad \text { hidden state } \\
& \overline{z_{t}}=\varphi\left(\tilde{z}_{t}\right), \quad \tilde{z}_{t}=\boldsymbol{w}_{z}^{\top} \boldsymbol{x}_{t}+r_{z} h_{t-1}+b_{z} \quad \text { cell input } \\
& \overline{\mathrm{i}}_{t}=\sigma\left(\tilde{\mathrm{i}}_{t}\right), \quad \tilde{\mathrm{i}}_{t}=\boldsymbol{w}_{\mathrm{i}}^{\top} \boldsymbol{x}_{t}+r_{\mathrm{i}} h_{t-1}+b_{\mathrm{i}} \quad \text { input gate } \\
& \mathrm{f}_{t}=\sigma\left(\tilde{\mathrm{f}}_{t}\right), \quad \tilde{\mathrm{f}}_{t}=\boldsymbol{w}_{t}^{\top} \boldsymbol{x}_{t}+r_{t} h_{t-1}+b_{t} \quad \text { forget gate } \\
...
& n_{t}=\mathrm{f}_{t} n_{t-1}+\mathrm{i}_{t} \\
& h_{t}=\mathrm{o}_{t} \tilde{h}_{t}, \quad \tilde{h}_{t}=c_{t} / n_{t} \\
& z_{t}=\varphi\left(\tilde{z}_{t}\right), \quad \tilde{z}_{t}=\boldsymbol{w}_{z}^{\top} \boldsymbol{x}_{t}+r_{z} h_{t-1}+b_{z} \\
& \mathrm{i}_{t}=\exp \left(\tilde{\mathrm{i}}_{t}\right), \quad \tilde{\mathrm{i}}_{t}=\boldsymbol{w}_{i}^{\top} \boldsymbol{x}_{t}+r_{\mathrm{i}} h_{t-1}+b_{\mathrm{i}} \\
& \mathrm{f}_{t}=\sigma\left(\tilde{\mathrm{f}}_{t}\right) \text { OR } \exp \left(\tilde{\mathrm{f}}_{t}\right), \quad \tilde{\mathrm{f}}_{t}=\boldsymbol{w}_{t}^{\top} \boldsymbol{x}_{t}+r_{t} h_{t-1}+b_{f} \\
& \mathrm{o}_{t}=\sigma\left(\tilde{\mathrm{o}}_{t}\right), \quad \tilde{\mathrm{o}}_{t}=\boldsymbol{w}_{\mathrm{o}}^{\top} \boldsymbol{x}_{t}+r_{\mathrm{o}} h_{t-1}+b_{\mathrm{o}}
\end{aligned}
$$
...
$$
\begin{aligned}
m_{t} & =\max \left(\log \left(\mathrm{f}_{t}\right)+m_{t-1} \cdot \log \left(\mathrm{i}_{t}\right)\right) \\
\mathrm{i}_{t}^{\prime} & =\exp \left(\log \left(\mathrm{i}_{t}\right)-m_{t}\right)=\exp \left(\tilde{\mathrm{i}}_{t}-m_{t}\right) \\
\mathrm{f}_{t}^{\prime} & =\exp \left(\log \left(\mathrm{f}_{t}\right)+m_{t-1}-m_{t}\right)
\end{aligned}
$$
...
We show in Appendix A.2, that replacing $\mathrm{f}_{t}$ by $\mathrm{f}_{t}^{\prime}$ and $\mathrm{i}_{t}$ by $\mathrm{i}_{t}^{\prime}$ in the forward pass does neither change the output of the whole network nor the derivatives of the loss with respect to the parameters.

New Memory Mixing. sLSTM can have multiple memory cells like the original LSTM (see Appendix A.2). Multiple memory cells enable memory mixing via recurrent connections $\boldsymbol{R}_{\boldsymbol{z}}, \boldsymbol{R}_{\mathbf{i}}$, $\boldsymbol{R}_{\mathbf{f}}, \boldsymbol{R}_{\mathbf{o}}$ from hidden state vector $\boldsymbol{h}$ to memory cell input $\boldsymbol{z}$ and the gates $\mathbf{i}, \mathbf{f}, \mathbf{o}$, respectively. A new aspect in memory mixing is the effect of exponential gating. The new sLSTM can have multiple heads with memory mixing within each head but not across heads. The introduction of heads for sLSTM together with exponential gating establishes a new way of memory mixing.
...
\boldsymbol{h}_{t} & =\mathbf{o}_{t} \odot \tilde{\boldsymbol{h}}_{t}, \quad \tilde{\boldsymbol{h}}_{t}=\boldsymbol{C}_{t} \boldsymbol{q}_{t} / \max \left\{\left\|\boldsymbol{n}_{t}^{\top} \boldsymbol{q}_{t}\right\|, 1\right\} \\
...
Figure 3: xLSTM blocks. Left: A residual sLSTM block with post up-projection (like Transformers): The input is fed into an sLSTM - with an optional convolution - followed by a gated MLP. Right: A residual mLSTM block with pre up-projection (like State Space models): mLSTM is wrapped inside two MLPs, via a convolution, a learnable skip connection, and an output gate that acts componentwise. See Figure 10 and Figure 11 in the appendix for details. space; see left panel of Figure 3 and third column in Figure 1. A more detailed version is depicted in Figure 10 in the appendix. (ii) A residual block with pre up-projection (like State Space Models), which linearly maps to a high-dimensional space, non-linearly summarizes the past in the high-dimensional space and then linearly maps back to the original space. For an xLSTM block containing an sLSTM, we use the post up-projection block. For an xLSTM block containing an mLSTM, we use the pre up-projection block since the memory capacity becomes larger in the high-dimensional space. We refer to the left panel of Figure 3 and third column in Figure 1, or Figure 10 in the appendix for more details.

**Images on this page:** 1
- Image ID: img-2.jpeg (Position: 722,1126 to 1421,1493)
...
**Images on this page:** 2
- Image ID: img-3.jpeg (Position: 302,200 to 1394,803)
- Image ID: img-4.jpeg (Position: 302,1559 to 1394,1887)
...
| Model | \#Params <br> M | SlimPajama <br> $(15 B) \mathrm{ppl} \downarrow$ |
| :-- | :--: | :--: |
| GPT-3 | 356 | 14.26 |
...
| Model | Modification | Exponential <br> Gating | Matrix <br> Memory | \#Params <br> M | SlimPajama <br> (15B) ppl $\downarrow$ |
| :--: | :--: | :--: | :--: | :--: | :--: |
...
| xLSTM[0:1] | Adding Exponential Gating | $\checkmark$ | $\boldsymbol{X}$ | 427.3 | 17.70 |
| xLSTM[7:1] | Adding Matrix Memory | $\checkmark$ | $\checkmark$ | 408.4 | 13.48 |

Ablation studies on different gating techniques.

| Learnable Gates | Forget Gate |  |  | Input Gate |  |  | SlimPajama <br> (15B) ppl $\downarrow$ |
| :--: | :--: | :--: | :--: | :--: | :--: | :--: | :--: |
|  | Input <br> Dependent | Learnable Bias | Bias <br> Init | Input Dependent | Learnable Bias | Bias <br> Init |  |
| No Gates | $\boldsymbol{X}$ | $\boldsymbol{X}$ | $+\infty$ | $\boldsymbol{X}$ | $\boldsymbol{X}$ | 0 | NaN |
| No Gates | $\boldsymbol{X}$ | $\boldsymbol{X}$ | $[3,6]$ | $\boldsymbol{X}$ | $\boldsymbol{X}$ | 0 | 13.95 |
| Forget Gate | $\checkmark$ | $\checkmark$ | $[3,6]$ | $\boldsymbol{X}$ | $\boldsymbol{X}$ | 0 | 13.58 |
| Input Gate | $\boldsymbol{X}$ | $\boldsymbol{X}$ | $[3,6]$ | $\checkmark$ | $\checkmark$ | $\mathcal{N}(0,0.1)$ | 13.69 |
| Forget Gate Bias | $\boldsymbol{X}$ | $\checkmark$ | $[3,6]$ | $\boldsymbol{X}$ | $\boldsymbol{X}$ | 0 | 13.76 |
| Forget + Input Gate Bias | $\boldsymbol{X}$ | $\checkmark$ | $[3,6]$ | $\boldsymbol{X}$ | $\checkmark$ | $\mathcal{N}(0,0.1)$ | 13.73 |
| Forget Gate + Input Gate Bias | $\checkmark$ | $\checkmark$ | $[3,6]$ | $\boldsymbol{X}$ | $\checkmark$ | $\mathcal{N}(0,0.1)$ | 13.55 |
...
**Images on this page:** 1
- Image ID: img-5.jpeg (Position: 287,264 to 1021,803)
...
**Images on this page:** 1
- Image ID: img-6.jpeg (Position: 290,770 to 1421,1240)
...
|  | Model | #Params <br> M | SlimPajama <br> (300B) ppl $\downarrow$ | LAMBADA <br> ppl $\downarrow$ | LAMBADA <br> acc ? | HellaSweg <br> acc ? | $\begin{aligned} & \text { PIQA } \\ & \text { acc ? } \end{aligned}$ | ARC-E <br> acc ? | ARC-C <br> acc ? | WinoGrande acc ? | Average acc ? |
| :--: | :--: | :--: | :--: | :--: | :--: | :--: | :--: | :--: | :--: | :--: | :--: |
| $\stackrel{\rightharpoonup}{\beta}$ | RWKV-4 | 169.4 | 16.66 | 54.72 | 23.77 | 34.03 | 66.00 | 47.94 | 24.06 | 50.91 | 41.12 |
...
|  | Model | #Params <br> M | C4 | MC4 <br> EN | Wikitext <br> 103 | Penn <br> Treebank | Red <br> Pajama | Refined <br> Web | Dolma | M2D2 <br> X2ORC | M2D2 <br> Wikipedia | C4 <br> Domains | Dolma <br> Subtraffic | Dolma <br> Coding | Average |
| :--: | :--: | :--: | :--: | :--: | :--: | :--: | :--: | :--: | :--: | :--: | :--: | :--: | :--: | :--: | :--: |
| $\stackrel{\rightharpoonup}{\beta}$ | RWKV-4 | 169.4 | 26.25 | 22.33 | 29.18 | 38.45 | 8.99 | 32.47 | 17.04 | 23.86 | 21.42 | 22.68 | 37.08 | 5.12 | 23.74 |
...
|  | Mamba | 423.1 | 17.33 | 13.05 | 16.11 | 22.24 | 6.34 | 21.04 | 11.42 | 14.83 | 14.53 | 15.36 | 27.02 | 3.20 | 15.19 |
...
|  | Mamba | 870.5 | 15.08 | 11.54 | 13.47 | 19.34 | 5.69 | 18.43 | 10.15 | 13.05 | 12.62 | 13.25 | 23.94 | 2.99 | 13.36 |
...
|  | xLSTM[7:1] | 1420.1 | 13.31 | 10.21 | 11.32 | 16.00 | 5.16 | 16.48 | 9.11 | 11.61 | 11.10 | 11.76 | 21.56 | 2.75 | 11.69 |
...
**Images on this page:** 1
- Image ID: img-7.jpeg (Position: 306,730 to 1387,1381)
...
**Images on this page:** 1
- Image ID: img-8.jpeg (Position: 294,189 to 1387,587)
...
& \boldsymbol{h}_{t}=\mathbf{o}_{t} \odot \hat{\boldsymbol{h}}_{t}, \quad \hat{\boldsymbol{h}}_{t}=\psi\left(\mid \boldsymbol{c}_{t}\right) \quad \text { hidden state } \\
& \boldsymbol{z}_{t}=\varphi\left(\tilde{\boldsymbol{z}}_{t}\right), \quad \tilde{\boldsymbol{z}}_{t}=\boldsymbol{W}_{\boldsymbol{z}} \boldsymbol{x}_{t}+\boldsymbol{R}_{\boldsymbol{z}} \boldsymbol{h}_{t-1}+\boldsymbol{b}_{\boldsymbol{z}} \quad \text { cell input } \\
...
& \mathbf{f}_{t}=\sigma\left(\tilde{\mathbf{f}}_{t}\right), \quad \tilde{\mathbf{f}}_{t}=\boldsymbol{W}_{\mathbf{f}} \boldsymbol{x}_{t}+\boldsymbol{R}_{\mathbf{f}} \boldsymbol{h}_{t-1}+\boldsymbol{b}_{\mathbf{f}} \quad \text { forget gate } \\
...
\begin{aligned}
& \boldsymbol{c}_{t}=\mathbf{f}_{t} \odot \boldsymbol{c}_{t-1}+\mathbf{i}_{t} \odot \boldsymbol{z}_{t} \quad \text { cell state } \\
& \boldsymbol{n}_{t}=\mathbf{f}_{t} \odot \boldsymbol{n}_{t-1}+\mathbf{i}_{t} \quad \text { normalizer state } \\
& \boldsymbol{h}_{t}=\mathbf{o}_{t} \odot \hat{\boldsymbol{h}}_{t}, \quad \hat{\boldsymbol{h}}_{t}=\boldsymbol{c}_{t} \odot \boldsymbol{n}_{t}^{-1} \quad \text { hidden state } \\
& \boldsymbol{z}_{t}=\varphi\left(\tilde{\boldsymbol{z}}_{t}\right), \quad \tilde{\boldsymbol{z}}_{t}=\boldsymbol{W}_{\boldsymbol{z}} \boldsymbol{x}_{t}+\boldsymbol{R}_{\boldsymbol{z}} \boldsymbol{h}_{t-1}+\boldsymbol{b}_{\boldsymbol{z}} \quad \text { cell input } \\
& \mathbf{i}_{t}=\exp \left(\tilde{\mathbf{i}}_{t}\right), \quad \tilde{\mathbf{i}}_{t}=\boldsymbol{W}_{\mathbf{i}} \boldsymbol{x}_{t}+\boldsymbol{R}_{\mathbf{i}} \boldsymbol{h}_{t-1}+\boldsymbol{b}_{\mathbf{i}} \quad \text { input gate } \\
& \mathbf{f}_{t}=\exp \left(\tilde{\mathbf{f}}_{t}\right) \text { OR } \sigma\left(\tilde{\mathbf{f}}_{t}\right), \quad \tilde{\mathbf{f}}_{t}=\boldsymbol{W}_{\mathbf{f}} \boldsymbol{x}_{t}+\boldsymbol{R}_{\mathbf{f}} \boldsymbol{h}_{t-1}+\boldsymbol{b}_{\mathbf{f}} \quad \text { forget gate } \\
& \mathbf{o}_{t}=\sigma\left(\tilde{\mathbf{o}}_{t}\right), \quad \tilde{\mathbf{o}}_{t}=\boldsymbol{W}_{\mathbf{o}} \boldsymbol{x}_{t}+\boldsymbol{R}_{\mathbf{o}} \boldsymbol{h}_{t-1}+\boldsymbol{b}_{\mathbf{o}} \quad \text { output gate }
\end{aligned}
$$
...
$$
\begin{aligned}
\delta_{\boldsymbol{h}_{t}} & =\delta_{\boldsymbol{h}_{t}}^{c x t}+\delta_{\boldsymbol{h}_{t}}^{\boldsymbol{R}} \\
...
\delta_{\boldsymbol{n}_{t-1}} & =\mathbf{f}_{t} \odot \delta_{\boldsymbol{n}_{t}}-\mathbf{o}_{t-1} \odot \boldsymbol{c}_{t-1} \odot \boldsymbol{n}_{t-1}^{-2} \odot \delta_{\boldsymbol{h}_{t-1}} \\
...
Recurrent mLSTM Backward Pass. The recurrent formulation of the mLSTM cell in Equation 19 yields the following backward pass recurrence, where $\delta_{a}$ indicates gradients with respect to parameter or internal variable $a$ and $\delta_{\boldsymbol{h}_{t}}^{\text {ext }}$ denotes gradients from subsequent layers:

$$
\begin{aligned}
\delta_{\tilde{\boldsymbol{h}}_{t}} & =\mathbf{o}_{t} \odot \delta_{\boldsymbol{h}_{t}}^{\text {ext }} \\
\delta_{\boldsymbol{C}_{t-1}}^{\top} & =\mathrm{f}_{t} \delta_{\boldsymbol{C}_{t}}^{\top}+\frac{\boldsymbol{q}_{t-1} \delta_{\tilde{\boldsymbol{h}}_{t-1}}^{\top}}{\max \left\{\left|\boldsymbol{n}_{t-1}^{\top} \boldsymbol{q}_{t-1}\right|, 1\right\}} \\
\delta_{\boldsymbol{n}_{t-1}} & =\mathrm{f}_{t} \delta_{\boldsymbol{n}_{t}}-\frac{\boldsymbol{q}_{t-1}^{\top} \boldsymbol{C}_{t-1}^{\top} \delta_{\tilde{\boldsymbol{h}}_{t-1}}}{\max \left\{\left|\boldsymbol{n}_{t-1}^{\top} \boldsymbol{q}_{t-1}\right|, 1\right\}^{2}} \Omega\left(\boldsymbol{n}_{t-1}^{\top} \boldsymbol{q}_{t-1}\right) \boldsymbol{q}_{t-1} \\
...
\delta_{\tilde{\mathrm{i}}_{t}} & =\left(\mathbf{1}^{\top}\left(\boldsymbol{C}_{t-1} \odot \delta_{\boldsymbol{C}_{t}}\right) \mathbf{1}+\mathbf{1}^{\top}\left(\boldsymbol{n}_{t-1} \odot \delta_{\boldsymbol{n}_{t}}\right)\right) \gamma\left(\tilde{\mathrm{f}}_{t}\right) \\
\delta_{\tilde{\mathbf{i}}_{t}} & =\left(\mathbf{1}^{\top}\left(\left(\boldsymbol{v}_{t} \boldsymbol{k}_{t}^{\top}\right) \odot \delta_{\boldsymbol{C}_{t}}\right) \mathbf{1}+\mathbf{1}^{\top}\left(\boldsymbol{k}_{t} \odot \delta_{\boldsymbol{n}_{t}}\right)\right) \exp \left(\tilde{\mathrm{i}}_{t}\right) \\
...
$$
\mathbf{F}_{i j}= \begin{cases}0 & \text { for } i<j \\ 1 & \text { for } i=j \\ \prod_{k=j+1}^{i} \sigma\left(\tilde{\mathrm{f}}_{k}\right) & \text { for } i>j\end{cases}
$$

and the input gate pre-activation matrix $\tilde{\mathbf{I}} \in \mathbb{R}^{T \times T}$ by

$$
\tilde{\mathbf{I}}_{i j}= \begin{cases}0 & \text { for } i<j \\ \tilde{\mathrm{i}}_{j} & \text { for } i \geqslant j\end{cases}
$$
...
$$
\widetilde{\mathbf{H}}=\boldsymbol{C} \boldsymbol{V}, \quad \text { with } \boldsymbol{C}=\frac{\widetilde{\boldsymbol{C}}}{\max \left(\left|\sum_{j=1}^{T} \widetilde{\boldsymbol{C}}_{i j}\right|, 1\right)}, \quad \text { and } \widetilde{\boldsymbol{C}}=\frac{\boldsymbol{Q} \boldsymbol{K}^{\top}}{\sqrt{d}} \odot \mathbf{D}
$$
...
$$
\widetilde{\mathbf{H}}=\boldsymbol{C} \boldsymbol{V}, \quad \text { with } \boldsymbol{C}=\frac{\widetilde{\boldsymbol{C}}^{\prime}}{\max \left(\left|\sum_{j=1}^{T} \widetilde{\boldsymbol{C}}_{i j}^{\prime}\right|, \exp (-\max \widetilde{\mathbf{D}})\right)}, \quad \text { and } \widetilde{\boldsymbol{C}}^{\prime}=\frac{\boldsymbol{Q} \boldsymbol{K}^{\top}}{\sqrt{d}} \odot \mathbf{D}^{\prime}
$$
...
$$
\begin{aligned}
\widetilde{\mathbf{D}} & =\overline{\mathbf{F}}+\tilde{\mathbf{1}} \\
...
\boldsymbol{n} & =\max (|\boldsymbol{b}|, \exp (-\boldsymbol{m})) \\
...
$$
\begin{aligned}
\delta_{\boldsymbol{C}}^{\top} & =\boldsymbol{V} \delta_{\tilde{\mathbf{H}}} \\
\delta_{\boldsymbol{n}} & =-\left(\widetilde{\boldsymbol{C}}^{\prime} \odot\left(\boldsymbol{n}^{-2} \mathbf{1}^{\top}\right) \odot \delta_{\boldsymbol{C}}\right) \mathbf{1} \\
& =-\left(\left(\widetilde{\boldsymbol{C}}^{\prime} \odot \delta_{\boldsymbol{C}}\right) \mathbf{1}\right) \odot \boldsymbol{n}^{-2} \\
\delta_{\boldsymbol{b}} & =\operatorname{sign}(\boldsymbol{n}) \odot \delta_{\boldsymbol{n}} \odot\left\{\begin{array}{ll}
1 & \text { if }|\boldsymbol{b}|>\exp (-\mathbf{m}) \\
0 & \text { otherwise }
\end{array}\right. \\
\delta_{\widetilde{\boldsymbol{C}}^{\prime}, \boldsymbol{C}} & =\left(\boldsymbol{n}^{-1} \mathbf{1}^{\top}\right) \odot \delta_{\boldsymbol{C}}, \\
\delta_{\widetilde{\boldsymbol{C}}^{\prime}, \boldsymbol{b}} & =\mathbf{1} \delta_{\boldsymbol{b}}^{\top}, \\
\delta_{\widetilde{\boldsymbol{C}}^{\prime}} & =\delta_{\widetilde{\boldsymbol{C}}^{\prime}, \boldsymbol{C}}+\delta_{\widetilde{\boldsymbol{C}}^{\prime}, \boldsymbol{B}} \\
\delta_{\mathbf{D}^{\prime}} & =\frac{\boldsymbol{Q} \boldsymbol{K}^{\top}}{\sqrt{d}} \odot \delta_{\widetilde{\boldsymbol{C}}^{\prime}} \\
\delta_{\widetilde{\mathbf{D}}} & =\exp (\widetilde{\mathbf{D}}-\boldsymbol{m}) \odot \delta_{\mathbf{D}^{\prime}}=\mathbf{D}^{\prime} \odot \delta_{\mathbf{D}^{\prime}}
\end{aligned}
$$
...
$$
\begin{aligned}
\delta_{\overline{\mathbf{F}}} & =\delta_{\widetilde{\mathbf{D}}} \\
...
\delta_{\boldsymbol{V}} & =\boldsymbol{C}^{\top} \delta_{\tilde{\mathbf{H}}}
\end{aligned}
$$

Having computed the gradients for the logarithmic forget gate matrix $\delta_{\overline{\mathbf{F}}}$, we can compute the gradients for the forget gate pre-activations $\delta_{\overline{\mathbf{f}}}=\left[\delta_{\overline{\mathrm{f}}_{1}}, \delta_{\overline{\mathrm{f}}_{2}}, \ldots, \delta_{\overline{\mathrm{f}}_{T}}\right]^{\top} \in \mathbb{R}^{T}$.
...
$$
\overline{\mathbf{F}}_{i j}=\log \mathbf{F}_{i j}= \begin{cases}-\infty & \text { for } i<j \\ 0 & \text { for } i=j \\ \sum_{k=j+1}^{i} \underbrace{\log \sigma\left(\tilde{\mathrm{f}}_{k}\right)}_{=\tilde{\mathrm{f}}_{k}}=\sum_{k=j+1}^{i} \tilde{\mathrm{f}}_{k} & \text { for } i>j\end{cases}
$$

With the substitution $\overline{\mathbf{f}}=\log \sigma(\tilde{\mathbf{f}})$ we compute the gradients for the logarithmic forget gate activations $\delta_{\tilde{\mathbf{f}}}=\left[\delta_{\tilde{\mathrm{f}}_{1}}, \delta_{\tilde{\mathrm{f}}_{2}}, \ldots, \delta_{\tilde{\mathrm{f}}_{T}}\right]^{\top} \in \mathbb{R}^{T}$ as

$$
\begin{aligned}
& \delta_{\tilde{\mathrm{f}}_{k}}=\sum_{j=1}^{k-1} \sum_{i=k}^{T}\left(\delta_{\overline{\mathbf{F}}}\right)_{i j} \\
& \delta_{\tilde{\mathrm{f}}_{k}}=\sigma\left(-\tilde{\mathrm{f}}_{k}\right) \cdot \delta_{\tilde{\mathrm{f}}_{k}}
\end{aligned}
$$
...
Finally, we compute the input gate pre-activations' gradients $\delta_{\tilde{\mathbf{i}}}=\left[\delta_{\tilde{i}_{1}}, \delta_{\tilde{i}_{2}}, \ldots, \delta_{\tilde{i}_{S}}\right]^{\top} \in \mathbb{R}^{T}$ as the column-wise sum over the rows of the input gate matrix $\delta_{\mathbf{I}}$ :

$$
\delta_{\tilde{i}_{k}}=\sum_{i=k}^{T}\left(\delta_{\mathbf{I}}\right)_{i k}
$$

This completes the backward pass of the parallel mLSTM for a full input sequence $\boldsymbol{X} \in \mathbb{R}^{T \times d}$.
...
**Images on this page:** 1
- Image ID: img-9.jpeg (Position: 306,259 to 1387,1696)
...
**Images on this page:** 1
- Image ID: img-10.jpeg (Position: 306,187 to 1387,1452)
...
All models are trained with 3 different learning rates (1e-2, 1e-3, 1e-4), each with two seeds. Batch size is 256 - cosine annealing (min lr: 1e-5) with $10 \%$ warm-up steps is applied. We use AdamW (Loshchilov \& Hutter, 2019) $\left(\beta_{1}=0.9, \beta_{2}=0.99\right)$ and a weight decay of 0.1 for training.

[^0]
[^0]:    ${ }^{1}$ https://python.org
    ${ }^{2}$ https://pytorch.org
...
**Images on this page:** 1
- Image ID: img-11.jpeg (Position: 501,193 to 1196,893)
...
- Mod Arithmetic (w/o Brackets) Calculate the result — modulo the max number — of the arithmetic operations in the context. The maximum number is the vocabulary size minus the number of special tokens (+,$\cdot$,*,=, [PAD]).
$|V|=10 \quad s_{\text {rand }}=\frac{1}{|V|-5}$
- Mod Arithmetic (w Brackets) Calculate the result — modulo the maximum number — of the arithmetic operations in the context. The maximum number is vocabulary size minus the number of special tokens (+,$\cdot$,*,=,(,), [PAD]).
$|V|=12 \quad s_{\text {rand }}=\frac{1}{|V|-7}$
- Odds First An string of tokens $t_{1}, t_{2}, t_{3}, \ldots t_{n}$ is given. Output all tokens with and odd index $\left(t_{1}, t_{3}, \ldots\right)$ then the token with an even index $\left(t_{2}, t_{4}, ..\right)$. Apart from that keep the ordering of the initial string.
...
$|V|=11 \quad s_{\text {rand }}=\frac{1}{\left\lfloor\frac{|V|-3}{2}\right\rfloor}$
...
- Solve Equation Given is an equation with the operators $\{+,-, *,=,(,)\}$, number, and an unknown variable x. Compute the value of the variable modulo the max number. The maximum number is vocabulary size minus the number of special tokens (+,$\cdot$,*,=,(,), [PAD], $[\mathrm{ACT}]$ ).
...
```
Bucket Sort
    Sequence: 1 4 8 6 1 1 1 1 4 6 8
Cycle Nav
...
For each task setup, we train each model with 4 different learning rates (batch size $>24$ : $\{1 \mathrm{e}-2$, $2.15 \mathrm{e}-3,4.6 \mathrm{e}-4,1 \mathrm{e}-4\}$, batch size 24 : $\{1 \mathrm{e}-3,2.2 \mathrm{e}-4,5 \mathrm{e}-5,1 \mathrm{e}-5\}$ ). The batch size (BS) changes depending on the context length (CL) (CL=64/128: BS=512; CL=256: BS=256; CL=756: BS=128; $\mathrm{CL}=1024: \mathrm{BS}=96 ; \mathrm{CL}=2048: \mathrm{BS}=24$ ). We vary the embedding dimension (Model Dim) between different experiments - different numbers of heads are used accordingly. For each experiment, we generate 100,000 training samples (validation: 3,000 samples) and train for 64 epochs. We apply cosine annealing (min lr: 1e-4 and 1e-5) with $10 \%$ warm-up steps. We use AdamW (Loshchilov \& Hutter, 2019) and a weight decay of 0.1 for training.
We conduct three different experiments:

- MQAR-Experiment 1 evaluates, in the same fashion as Arora et al. (2023), a variety of models (Llama, Mamba, Mamba (noWT) - i.e. without weight tying, Retention, Hyena, H3, RWKV-4, RWKV-5, RWKV-6, LSTM, LSTM (Block), xLSTM[0:1], $\mathrm{xLSTM}[1: 0]$ and $\mathrm{xLSTM}[1: 1]$ ) on increasing task difficulty by increasing the context length and number of key-value pairs simultaneously. We benchmark three parameter settings: $\mathrm{CL}, \mathrm{KV}=\{(64,4),(128,8),(256,16)\}$.
- MQAR-Experiment 2 increases the task difficulty notably and goes beyond previous evaluations on this task. We individually scale the context length (CL=\{756, 1024, 2048\}) and the key-value pairs $(\mathrm{KV}=\{48,96,256\})$ and evaluate all combinations. This experiment especially probes the memory capacity because the number of key-value pairs is high. To reduce the computational burden we only evaluate models that perform flawlessly in Experiment 1 - additionally we evaluate Transformer only in the hardest setting (CL=2048) as sanity check, because no performance decrease is expected.
...
[^0]
[^0]:    ${ }^{6}$ The keys are distributed on the "evaluation part" of the sequence given a power-law distribution. This is motivated by similar structures in natural language text.
...
- ListOps: This is a set of modular arithmetic tasks including brackets and lists of numbers, using the operations MIN, MAX, MEDIAN and SUMMOD (modular sum). A particular example is: [MAX 43 [MIN 23 ] 10 [MEDIAN 1589 , 2]] $\rightarrow 5$
...
We use the hyperparameter settings of the S5 model (Smith et al., 2022) and Linear Recurrent Unit model (Orvieto et al., 2023), with additional hyperparamter search on learning rates and schedulers for all models. We use two different schedulers: Linear Warm-up Cosine Annealing and Linear Warm-up Cosine Annealing with Restarts. Both learning rate schedulers were evaluated with learning rates of $1 \mathrm{e}-3,6 \mathrm{e}-4$ and $1 \mathrm{e}-4$. For the second scheduler, the number of restarts $(R)$ is set to 3 . The model hyperparameters for each dataset are displayed in Table 5.
...
**Images on this page:** 1
- Image ID: img-12.jpeg (Position: 289,200 to 1409,409)
...
**Images on this page:** 1
- Image ID: img-13.jpeg (Position: 287,294 to 1409,1790)
...
**Images on this page:** 1
- Image ID: img-14.jpeg (Position: 290,294 to 1404,1185)
...
**Images on this page:** 1
- Image ID: img-15.jpeg (Position: 290,286 to 1404,1056)
...
General Training Procedure. We tokenize our datasets using the HuggingFace GPT-2 tokenizer (Radford et al., 2019; Brown et al., 2020) ${ }^{5}$ and use this tokenizer for all models in this paper. In general, we try to follow Brown et al. (2020) for the general training setup, i.e. we choose context length 2048 and batch sizes 256 or 512 for our models. We use the AdamW (Loshchilov \& Hutter, 2019) optimizer with beta parameters $\left(\beta_{1}, \beta_{2}\right)=(0.9,0.95)$ and an epsilon parameter of $1 \mathrm{e}-5$. As learning rate scheduler we use a linear warm-up with 750 steps and cosine decay to $10 \%$ of the peak learning rate. We apply a weight decay of 0.1 to all our models and always exclude the token embedding matrix from weight decay. If not specified otherwise, we do not tie the weights of the token embedding and the language model head. For parallelization, we use PyTorch FSDP in SHARD_GRAD_OP mode with mixed precision in bfloat16, where applicable. For small models we use NO_SHARD. We keep the weights in float32 and reduce the gradients across GPUs in float32. We use torch. compile to speed up models, except for Transformer models as their training curves did not match the non-compiled versions. For xLSTM[7:1], we use positions [3, 5, 7, 40, 42, 44] for sLSTM-based blocks, except for the 125 M size, where we use [3, 20] (this is actually a [11:1] ratio). We do not use any positional encoding for our xLSTM models.

Details on Comparison to Other Methods. For the model comparison on 15B training tokens of SlimPajama we train all models with context length 2048 and batch size 256. We use a peak learning rate of $1 \mathrm{e}-3$ for all models for comparability. The learning rate decays over 30k training steps. The models are compared after one epoch at training step 28170. As model implementations we use the original repositories' code for Mamba (Gu \& Dao, 2023) ${ }^{6}$, RWKV-5, RWKV-6 (Peng et al., 2024) ${ }^{7}$. For RWKV-4 we use a cleaned and validated re-implementation based on the original repo and kernels (Peng et al., 2023). In our RWKV-4 implementation we enable weight decay on all parameters except biases, the token embedding weight and all LayerNorm weights. For HGRN (Qin et al., 2023), GLA (Yang et al., 2023), HGRN2 (Qin et al., 2024) we use the a re-implementation flash-linear-attention (Yang \& Zhang, 2024) by the authors of GLA (Yang et al., 2023; Yang \& Zhang, 2024) ${ }^{8}$. For GPT-3 and Llama-like Transformers, we use our own implementations based on PyTorch. Note that for all xLSTMs, Transformers, Mamba and RWKV-4, we use Mixed Precision training with bfloat16 and weights in float32 precision. Following the general training procedure we use torch. compile for all models, except for Transformers and models using the flash-linear-attention library because of compilation problems.
As RWKV-6 performs worse than RWKV-5, we also train a model with peak learning rate $4 \mathrm{e}-4$, as reported in the original repository for 350 M parameter models ${ }^{9}$. This model reaches a perplexity of 16.38, worse than the 15.03 for the standard peak learning rate $1 \mathrm{e}-3$ as reported in Table 1. Similarly, we tested the repository learning rates for other model sizes and all performed worse than the ones we also use for xLSTM (see Table 7).

Details on Training Precision for Baselines. For models from flash-linear-attention and RWKV-5/6 models we found that PyTorch automatic mixed precision training did not work, but casting the model weights to float 32 initially with FSDP parameter precision bfloat16 led to a working configuration. In this setting, models perform better than in full bfloat16 training, where the weights are casted to bfloat16 initially as well. Full float32 training did not work because of the custom kernels that require bfloat16.

General Details on Ablation Studies. We follow our general training procedure and train all models with context length 2048, batch size 256 and peak learning rate $1 \mathrm{e}-3$. We report perplexity values on the validation set.

[^0]
[^0]:    ${ }^{5}$ https://huggingface.co/docs/transformers/en/model_doc/gpt2
...
| :--: | :--: | :--: | :--: | :--: | :--: | :--: | :--: |
| 125 M | RWKV-5 | 768 | 12 | - | 176.5 | $3 \mathrm{e}-3$ | - |
|  | RWKV-6 | 768 | 12 | - | 173.6 | $3 \mathrm{e}-3$ | - |
|  | HGRN2 | 768 | 12 | - | 162.2 | $3 \mathrm{e}-3$ | - |
|  | RWKV-4 | 768 | 12 | - | 169.4 | $3 \mathrm{e}-3$ | $6 \mathrm{e}-4$ |
|  | Llama | 768 | 12 | 12 / 64 | 162.2 | $3 \mathrm{e}-3$ | $3 \mathrm{e}-3$ |
|  | Mamba | 768 | 24 | - | 167.8 | $3 \mathrm{e}-3$ | $3 \mathrm{e}-3$ |
|  | xLSTM | 768 | 24 | 4 / 384 | 163.8 | $3 \mathrm{e}-3$ | $1.5 \mathrm{e}-3$ |
| 350 M | RKWV-5 | 1024 | 24 | - | 455.7 | $1 \mathrm{e}-3$ | - |
|  | RWKV-6 | 1024 | 24 | - | 441.6 | $1 \mathrm{e}-3$ | - |
|  | HGRN2 | 1024 | 24 | - | 411.4 | $1 \mathrm{e}-3$ | - |
|  | RWKV-4 | 1024 | 24 | - | 430.5 | $1 \mathrm{e}-3$ | $4 \mathrm{e}-4$ |
|  | Llama | 1024 | 24 | 16 / 64 | 406.6 | $1.5 \mathrm{e}-3$ | $1.5 \mathrm{e}-3$ |
|  | Mamba | 1024 | 48 | - | 423.1 | $1.5 \mathrm{e}-3$ | $1.5 \mathrm{e}-3$ |
|  | xLSTM | 1024 | 48 | 4 / 512 | 409.3 | $1 \mathrm{e}-3$ | $7.5 \mathrm{e}-4$ |
| 760 M | RWKV-5 | 1536 | 24 | - | 947.8 | $9 \mathrm{e}-4$ | - |
|  | RWKV-6 | 1536 | 24 | - | 907.7 | $9 \mathrm{e}-4$ | - |
|  | HGRN2 | 1536 | 24 | - | 834.2 | $9 \mathrm{e}-4$ | - |
|  | RWKV-4 | 1536 | 24 | - | 891.0 | $2 \mathrm{e}-3$ | $2.5 \mathrm{e}-4$ |
|  | Llama | 1536 | 24 | 16 / 96 | 834.1 | $1.25 \mathrm{e}-3$ | $1.25 \mathrm{e}-3$ |
|  | Mamba | 1536 | 48 | - | 870.5 | $1.25 \mathrm{e}-3$ | $1.25 \mathrm{e}-3$ |
|  | xLSTM | 1536 | 48 | 4 / 768 | 840.4 | $9 \mathrm{e}-4$ | $6.25 \mathrm{e}-4$ |
| 1.3 B | RWKV-5 | 2048 | 24 | - | 1616.0 | $9 \mathrm{e}-4$ | - |
|  | RWKV-6 | 2048 | 24 | - | 1537.5 | $9 \mathrm{e}-4$ | - |
|  | HGRN2 | 2048 | 24 | - | 1439.4 | $9 \mathrm{e}-4$ | - |
|  | RWKV-4 | 2048 | 24 | - | 1515.2 | $1 \mathrm{e}-3$ | $2 \mathrm{e}-4$ |
|  | Llama | 2048 | 24 | 16 / 128 | 1420.4 | $1 \mathrm{e}-3$ | $1 \mathrm{e}-3$ |
|  | Mamba | 2048 | 48 | - | 1475.3 | $1 \mathrm{e}-3$ | $1 \mathrm{e}-3$ |
|  | xLSTM | 2048 | 48 | 4 / 1024 | 1422.6 | $9 \mathrm{e}-4$ | $5 \mathrm{e}-4$ |
| $\mathrm{r} 4 \mathrm{~B}$ | RWKV-5 | 2048 | 24 | - | 3194.7 | $8 \mathrm{e}-4$ | - |
|  | RWKV-6 | 2048 | 24 | - | 3021.9 | $8 \mathrm{e}-4$ | - |
|  | HGRN2 | 2048 | 24 | - | 2795.4 | $8 \mathrm{e}-4$ | - |
|  | RWKV-4 | 2560 | 32 | - | 2984.8 | $8 \mathrm{e}-4$ | - |
|  | Llama | 2560 | 32 | 32 / 80 | 2779.5 | $8 \mathrm{e}-4$ | - |
|  | Mamba | 2560 | 64 | - | 2897.2 | $8 \mathrm{e}-4$ | - |
...
| Model | Details | \#Blocks | Embedding <br> Dim | \#Params <br> M | SlimPajama <br> $(15 \mathrm{~B}) \mathrm{ppl} \downarrow$ |
| :-- | :-- | :--: | :--: | :--: | :--: |
...
| :--: | :--: | :--: | :--: | :--: | :--: | :--: |
| $\begin{aligned} & \text { ㄴ } \\ & \text { ㄷ } \end{aligned}$ | RWKV-4 | 169.4 | 265.80 | 39.31 | 18.48 | 53.89 |
...
| $\begin{aligned} & \text { ㄴ } \\ & \text { ㄴ } \\ & \text { ㄷ } \end{aligned}$ | RWKV-4 | 430.5 | 216.17 | 30.25 | 13.82 | 42.25 |
...
[^0]
[^0]:    ${ }^{10}$ https://huggingface.co/datasets/allenai/paloma
...
| c4_100dom_val_51_disneyparksmomspanel.diss | 11.99 | 11.29 | 11.98 | 11.16 | 11.00 |
...
| dolma_100_proglang_val_25scss | 2.26 | 2.27 | 2.38 | 2.24 | 2.24 |
...
| Dataset <br> dolma_100_proglang_val_33_perl <br> dolma_100_proglang_val_34_diff <br> dolma_100_proglang_val_35_ini <br> dolma_100_proglang_val_36_jsx <br> dolma_100_proglang_val_37_haskell <br> dolma_100_proglang_val_38_gnuplot <br> dolma_100_proglang_val_39_postscript <br> dolma_100_proglang_val_40_groff <br> dolma_100_proglang_val_41_turtle <br> dolma_100_proglang_val_42_fortran <br> dolma_100_proglang_val_43_makefile <br> dolma_100_proglang_val_44_mathematica <br> dolma_100_proglang_val_45_pascal <br> dolma_100_proglang_val_46_common-lisp <br> dolma_100_proglang_val_47_gas <br> dolma_100_proglang_val_48_vhdl <br> dolma_100_proglang_val_49_julia <br> dolma_100_proglang_val_50_edn <br> dolma_100_proglang_val_51_visual-basic <br> dolma_100_proglang_val_52_powershell <br> dolma_100_proglang_val_53_g-code <br> dolma_100_proglang_val_54_ocaml <br> dolma_100_proglang_val_55_java-server-p <br> dolma_100_proglang_val_56_solidity <br> dolma_100_proglang_val_57_graphviz-dot <br> dolma_100_proglang_val_58_less <br> dolma_100_proglang_val_59_twig <br> dolma_100_proglang_val_60_asciidoc <br> dolma_100_proglang_val_61_groovy <br> dolma_100_proglang_val_62_llvm <br> dolma_100_proglang_val_63_hcl <br> dolma_100_proglang_val_64_html+erb <br> dolma_100_proglang_val_65_ erlang <br> dolma_100_proglang_val_66_elixir <br> dolma_100_proglang_val_67_eagle <br> dolma_100_proglang_val_68_arduino <br> dolma_100_proglang_val_69_coffeescript <br> dolma_100_proglang_val_70_toml <br> dolma_100_proglang_val_71_cuda <br> dolma_100_proglang_val_72_nix <br> dolma_100_proglang_val_73_smalltalk <br> dolma_100_proglang_val_74_cmake <br> dolma_100_proglang_val_75_actionscript <br> dolma_100_proglang_val_76_glsl <br> dolma_100_proglang_val_77_systemverilog <br> dolma_100_proglang_val_78_haxe <br> dolma_100_proglang_val_79_f\# <br> dolma_100_proglang_val_80_max <br> dolma_100_proglang_val_81_objective-c++ <br> dolma_100_proglang_val_82_standard-ml <br> dolma_100_proglang_val_83_dockerfile <br> dolma_100_proglang_val_84_emacs-lisp |

| Llama | Mamba | RWKV-4 | xLSTM[7:1] | xLSTM[1:0] |
| :--: | :--: | :--: | :--: | :--: |
| 2.57 | 2.62 | 3.01 | 2.59 | 2.55 |
| 2.87 | 2.95 | 3.43 | 2.89 | 2.86 |
| 3.91 | 4.16 | 4.90 | 4.05 | 3.98 |
| 1.83 | 1.84 | 1.95 | 1.83 | 1.82 |
| 2.94 | 3.07 | 3.73 | 3.02 | 2.95 |
| 2.65 | 2.88 | 3.36 | 2.81 | 2.77 |
| 19.09 | 19.52 | 19.56 | 18.66 | 18.64 |
| 6.13 | 6.32 | 7.45 | 6.22 | 6.21 |
| 2.35 | 2.45 | 3.17 | 2.39 | 2.35 |
| 2.32 | 2.39 | 2.83 | 2.35 | 2.31 |
| 2.93 | 3.01 | 3.51 | 2.86 | 2.82 |
| 10.34 | 11.34 | 13.24 | 10.49 | 10.71 |
| 4.18 | 4.81 | 5.49 | 4.17 | 4.27 |
| 2.56 | 2.71 | 3.32 | 2.62 | 2.58 |
| 2.49 | 2.73 | 3.59 | 2.57 | 2.53 |
| 3.91 | 4.06 | 4.69 | 3.92 | 3.90 |
| 3.25 | 3.36 | 4.05 | 3.30 | 3.26 |
| 1.99 | 2.10 | 2.67 | 2.04 | 2.03 |
| 2.42 | 2.49 | 2.72 | 2.37 | 2.38 |
| 4.08 | 4.16 | 4.50 | 3.86 | 3.89 |
| 2.26 | 2.66 | 3.29 | 2.44 | 2.37 |
| 3.06 | 3.29 | 4.22 | 3.19 | 3.13 |
| 2.10 | 2.11 | 2.31 | 2.06 | 2.09 |
| 4.09 | 4.41 | 5.28 | 4.05 | 4.10 |
| 2.17 | 2.48 | 3.54 | 2.32 | 2.29 |
| 2.24 | 2.26 | 2.33 | 2.22 | 2.22 |
| 1.81 | 1.81 | 1.91 | 1.80 | 1.79 |
| 5.33 | 5.50 | 6.84 | 5.43 | 5.34 |
| 2.12 | 2.15 | 2.41 | 2.13 | 2.11 |
| 2.26 | 2.40 | 3.25 | 2.31 | 2.23 |
| 2.52 | 2.56 | 2.96 | 2.52 | 2.48 |
| 2.10 | 2.09 | 2.23 | 2.08 | 2.07 |
| 2.84 | 2.98 | 3.87 | 2.88 | 2.85 |
| 2.93 | 2.99 | 3.58 | 2.91 | 2.90 |
| 5.35 | 6.90 | 10.75 | 5.64 | 5.76 |
| 3.37 | 3.40 | 3.81 | 3.28 | 3.28 |
| 2.80 | 2.85 | 3.27 | 2.80 | 2.77 |
| 7.76 | 7.62 | 8.44 | 7.53 | 7.58 |
| 2.15 | 2.21 | 2.56 | 2.19 | 2.16 |
| 7.80 | 7.84 | 9.03 | 7.88 | 7.83 |
| 9.32 | 9.61 | 12.60 | 9.47 | 9.20 |
| 1.87 | 1.86 | 2.02 | 1.84 | 1.81 |
| 2.45 | 2.54 | 2.88 | 2.46 | 2.46 |
| 2.40 | 2.42 | 2.72 | 2.36 | 2.32 |
| 2.53 | 2.66 | 3.17 | 2.58 | 2.55 |
| 2.74 | 2.81 | 3.20 | 2.77 | 2.76 |
| 2.89 | 3.02 | 3.53 | 2.93 | 2.88 |
| 1.59 | 1.62 | 1.80 | 1.61 | 1.61 |
| 2.18 | 2.19 | 2.40 | 2.17 | 2.16 |
| 3.57 | 4.05 | 4.79 | 3.81 | 3.77 |
| 4.08 | 4.17 | 4.37 | 4.01 | 4.05 |
| 3.83 | 3.83 | 4.44 | 3.80 | 3.72 |
...
| Dataset <br> dolma_100_proglang_val_85_scheme <br> dolma_100_proglang_val_86_clojure <br> dolma_100_proglang_val_87_handlebars <br> dolma_100_proglang_val_88_smarty <br> dolma_100_proglang_val_89_logos <br> dolma_100_proglang_val_90_stata <br> dolma_100_proglang_val_91_yacc <br> dolma_100_proglang_val_92_nimrod <br> dolma_100_proglang_val_93_tcl <br> dolma_100_proglang_val_94_viml <br> dolma_100_proglang_val_95_asp <br> dolma_100_proglang_val_96_protocol-buffer <br> dolma_100_proglang_val_97_r <br> dolma_100_proglang_val_98_cython <br> dolma_100_proglang_val_99_mediawiki <br> dolma_100_subreddits_val_00_AskReddit <br> dolma_100_subreddits_val_01_politics <br> dolma_100_subreddits_val_02_AmItheAsshole <br> dolma_100_subreddits_val_03_worldnews <br> dolma_100_subreddits_val_04_relationships <br> dolma_100_subreddits_val_05_relationship_adv <br> dolma_100_subreddits_val_06_news <br> dolma_100_subreddits_val_07_leagueof Legends <br> dolma_100_subreddits_val_08_todayilearned <br> dolma_100_subreddits_val_09_TwoXChromoso <br> dolma_100_subreddits_val_10_personalfinance <br> dolma_100_subreddits_val_11_changemyview <br> dolma_100_subreddits_val_12_unpopularopinio <br> dolma_100_subreddits_val_13_movies <br> dolma_100_subreddits_val_14_Games <br> dolma_100_subreddits_val_15_nba <br> dolma_100_subreddits_val_16_pics <br> dolma_100_subreddits_val_17_gaming <br> dolma_100_subreddits_val_18_soccer <br> dolma_100_subreddits_val_19_nfl <br> dolma_100_subreddits_val_20_explainlikeimfiv <br> dolma_100_subreddits_val_21_conspiracy <br> dolma_100_subreddits_val_22_atheism <br> dolma_100_subreddits_val_23_AskMen <br> dolma_100_subreddits_val_24_videos <br> dolma_100_subreddits_val_25_sex <br> dolma_100_subreddits_val_26_raisedbynarcissi <br> dolma_100_subreddits_val_27_NoStupidQuesti <br> dolma_100_subreddits_val_28_DestinyTheGam <br> dolma_100_subreddits_val_29_anime <br> dolma_100_subreddits_val_30_DnD <br> dolma_100_subreddits_val_31_ukpolitics <br> dolma_100_subreddits_val_32_funny <br> dolma_100_subreddits_val_33_europe <br> dolma_100_subreddits_val_34_canada <br> dolma_100_subreddits_val_35_Christianity <br> dolma_100_subreddits_val_36_SquaredCircle | Llama <br> 2.78 <br> 3.18 <br> 1.79 <br> 2.30 <br> 2.37 <br> 4.67 <br> 2.42 <br> 2.75 <br> 3.00 <br> 5.56 <br> 1.79 <br> 1.32 <br> 2.80 <br> 2.34 <br> 2.01 <br> 20.25 <br> 22.08 <br> 22.49 <br> 22.57 <br> 18.64 <br> 19.40 <br> 21.62 <br> 22.26 <br> 23.28 <br> 21.84 <br> 24.45 <br> 23.38 <br> 19.86 <br> 18.35 <br> 23.86 <br> 21.23 <br> 20.00 <br> 22.26 <br> 21.13 <br> 22.07 <br> 19.66 <br> 35.27 <br> 23.21 <br> 28.22 <br> 22.35 <br> 20.78 <br> 21.76 <br> 22.44 <br> 17.88 <br> 25.87 | Mamba <br> 2.86 <br> 3.30 <br> 1.79 <br> 2.35 <br> 2.58 <br> 2.58 <br> 5.08 <br> 2.48 <br> 2.48 <br> 2.87 <br> 2.87 <br> 3.16 <br> 5.76 <br> 1.79 <br> 1.79 <br> 1.31 <br> 2.92 <br> 2.39 <br> 2.10 <br> 19.29 <br> 20.38 <br> 20.70 <br> 21.30 <br> 21.30 <br> 21.82 <br> 23.61 <br> 18.76 <br> 17.21 <br> 22.53 <br> 20.18 <br> 19.04 <br> 21.24 <br> 20.13 <br> 21.30 <br> 22.48 <br> 18.59 <br> 18.58 <br> 22.51 <br> 21.30 <br> 22.80 <br> 19.45 <br> 20.70 <br> 22.10 <br> 22.44 <br> 21.21 <br> 17.02 <br> 24.31 | RWKV-4 <br> 3.40 <br> 4.00 <br> 1.88 <br> 2.58 <br> 2.98 <br> 6.85 <br> 2.87 <br> 3.63 <br> 3.95 <br> 7.21 <br> 1.90 <br> 1.38 <br> 3.66 <br> 2.69 <br> 2.48 <br> 2.12 <br> 20.38 <br> 22.07 <br> 22.07 <br> 22.89 <br> 22.77 <br> 18.89 <br> 19.68 <br> 19.68 <br> 22.51 <br> 22.52 <br> 23.60 <br> 21.82 <br> 24.61 <br> 23.61 <br> 20.17 <br> 18.59 <br> 18.59 <br> 21.43 <br> 20.11 <br> 22.51 <br> 21.29 <br> 21.30 <br> 22.48 <br> 19.87 <br> 36.13 <br> 23.46 <br> 28.78 <br> 21.19 <br> 22.80 <br> 19.45 <br> 20.70 <br> 22.10 <br> 22.44 <br> 21.21 <br> 22.44 <br> 17.02 <br> 25.83 | 2.84 <br> 3.17 <br> 1.78 <br> 2.30 <br> 2.46 <br> 4.85 <br> 2.44 <br> 2.43 <br> 2.77 <br> 3.02 <br> 5.55 <br> 1.77 <br> 1.77 <br> 1.32 <br> 2.81 <br> 2.35 <br> 2.35 <br> 2.12 <br> 19.14 <br> 20.83 <br> 20.61 <br> 21.60 <br> 21.27 <br> 21.50 <br> 21.23 <br> 17.86 <br> 17.86 <br> 18.63 <br> 18.46 <br> 21.17 <br> 32.46 <br> 32.04 <br> 21.28 <br> 21.10 <br> 19.02 <br> 19.02 <br> 17.64 <br> 18.99 <br> 22.32 <br> 22.04 <br> 20.35 <br> 20.87 <br> 21.85 <br> 20.47 <br> 22.86 <br> 22.19 <br> 22.03 <br> 22.03 <br> 18.62 <br> 18.62 <br> 17.32 <br> 17.03 <br> 22.54 <br> 20.13 <br> 18.94 <br> 18.94 <br> 21.04 <br> 19.98 <br> 21.20 <br> 21.02 <br> 18.68 <br> 18.52 <br> 33.37 <br> 21.04 <br> 26.39 <br> 26.72 <br> 26.39 <br> 21.10 <br> 19.23 <br> 20.52 <br> 20.52 <br> 21.09 <br> 16.94 <br> 24.03 |
...
| m2d2_s2orc_unsplit_val_eess_11 | 13.77 | 13.89 | 15.71 | 13.76 | 13.54 |
...
| m2d2_s2orc_unsplit_val_math_11 | 7.81 | 8.07 | 9.36 | 7.99 | 7.87 |
...
| m2d2_s2orc_unsplit_val_nlin_11 | 12.43 | 12.75 | 14.88 | 12.61 | 12.44 |
...
|  m2d2_s2orc_unsplit_val_physics_ll | 15.57 | 15.43 | 16.97 | 15.40 | 15.18  |
...
|  m2d2_s2orc_unsplit_val_q-bio_ll | 13.69 | 13.87 | 15.75 | 13.75 | 13.50  |
...
|  m2d2_s2orc_unsplit_val_q-fin_ll | 13.79 | 13.91 | 15.73 | 13.83 | 13.61  |
...
|  m2d2_s2orc_unsplit_val_stat_ll | 13.07 | 12.56 | 14.42 | 12.46 | 12.24  |
|  m2d2_s2orc_unsplit_val_supr-con | 11.57 | 11.66 | 13.13 | 11.53 | 11.30  |
|  m2d2_wikipedia_unsplit_val_Culture_and_the | 12.30 | 11.90 | 12.82 | 11.78 | 11.66  |
...
| pth_val | 15.92 | 16.65 | 19.37 | 16.00 | 15.92 |
